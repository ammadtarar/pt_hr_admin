...
function BoxRecompense(props) {
  ...
  return (
    <div className="box-annonce">
      <ul className="titre" style={{ width: "calc(100% - 290px)" }}>
        <li>
          <h4>{data.title}</h4>
        </li>
        <li className="date">
          <p>
            Créé le <span>{date(dateToFormat)}</span>
          </p>
        </li>
      </ul>
      <ul className="details">
        ...
        <li className="star">
          <p>
            <span>{data.referral_success_reward_value}</span>
            {data.referral_success_reward_type === "cash"
              ? data.cash_reward_currency
              : data.referral_success_reward_type === "points"
              ? " points"
              : ""}
          </p>
        </li>
      ...
      <ul className="infos">
        {checked === true ? (
          <li className="box-candidats-cooptes">
